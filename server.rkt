...
(define (admin request)
  ...
  (define (lambda-get-logs request)
    (after-auth request (lambda () (define-values (club) (formlet-process get-logs request))
                          (response/xexpr (base "查询积分记录结果" `(body (h1 "查询积分记录结果")
                                                                  ,(user-get club)))))))
  (define (lambda-add-user request)
    (after-auth request (lambda () (define-values (name password club) (formlet-process add-user))
  ...
  (define (lambda-update-user request)
    (after-auth request (lambda () (define-values (name newname password club) (formlet-process update-user))
  ...
  (define (lambda-delete-user request)
    (after-auth request (lambda () (define-values (name) (formlet-process delete-user))
                          (user-delete! name)
                          (admin (redirect/get)))))
...
(define (query request)
  (define (render-query embed/url)
    ...
    (if id
        (response/xexpr (base "社团详细" `(body (h1 "社团详细") @(map (lambda (club) `(div (h2 ,club) (p ,(club-score club)))) (string-split (user-club id) "/")))))
