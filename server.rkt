  ...
  (define (login-handler request)
    ...
    (match (cons username password)
          [(cons user pass) #:when (let ((realpass (user-password user))) (if realpass (pwhash-verify #f (string->bytes/utf-8 pass) realpass) #f))
                            (send/suspend/dispatch (lambda (embed/url) (response/xexpr `(html (head (meta ((http-equiv "refresh") (content ,(string-append "0;url=" (embed/url (if (equal? (user-club user) "admin") admin query))))))))
...
;run
(serve/servlet homepage #:command-line? #t #:servlet-path "/" #:port 8080 #:listen-ip #f #:stateless? #t)
