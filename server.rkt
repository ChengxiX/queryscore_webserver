...
(define (admin request)
  (define (after-auth embed/url request next) (if (eq? (user-club (request-id-cookie request #:name "identity" #:key secret-salt #:shelf-life 86400)) "admin") (next)
  ...
  (define (render-admin embed/url)
    (after-auth embed/url request (lambda ()
  ...
  (define (lambda-get-club request)
    (after-auth embed/url request (lambda () (response/xexpr (base "所有社团" `(body (h1 "所有社团")
                                                               ,(club-all)))))))
  ...
  (define (lambda-update-user request)
    (after-auth embed/url request (lambda ()(
                                   (define-values (name newname password club) (formlet-process update-user))
                                   (if (eq? newname "") '() (user-rename! name newname))
                                   (if (eq? club "") '() (user-reclub! name club))
                                   (if (eq? password "") '() (user-repassword! name (pwhash 'scrypt (string->bytes/utf-8 password) `((ln , (inexact->exact (+ 1 (round (* (random) 10)))))))))
                                   (admin (redirect/get))))))
  (define (lambda-delete-user request)
    (after-auth embed/url request (lambda ()(
                                   (define-values (name) (formlet-process delete-user))
                                   (user-delete! name)
                                   (admin (redirect/get))))))
  (define (lambda-add-club request)
    (after-auth embed/url request (lambda ()(
                                   (define-values (name score) (formlet-process add-club))
                                   (club-insert! name score)
                                   (admin (redirect/get))))))
  (define (lambda-update-club request)
    (after-auth embed/url request (lambda ()(
                                   (define-values (name newname) (formlet-process update-club))
                                   (club-rename! name newname)
                                   (admin (redirect/get))))))
  (define (lambda-add-log request)
    (after-auth embed/url request (lambda ()(
                                   (define-values (name score comment) (formlet-process add-log))
