...
(define (admin request)
  (define (after-auth request next) (if (equal? (user-club (request-id-cookie request #:name "identity" #:key secret-salt #:shelf-life 86400)) "admin") (next request)
  ...
  (define (lambda-update-user request)
    (after-auth request (lambda (request) (define-values (name newname password club) (formlet-process update-user request))
