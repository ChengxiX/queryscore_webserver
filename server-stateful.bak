...
(define (admin request)
  ...
  (define (lambda-get-club request)
    (lambda (request) (response/xexpr (base "所有社团" `(body (h1 "所有社团")
...
(define (query request)
  (define (render-query embed/url)
        (response/xexpr (base "社团详细" `(body (h1 "社团详细") ,@(map (lambda (club) `(div (h2 ,club) (p ,(number->string (club-score club))) ,(let ((res (log-*-byclub club))) (table-render-4 (first res) (second res) (map number->string (third res)) (fourth res))))) (string-split (user-club
                                                                                                                                                                                                             (request-id-cookie request #:name "identity" #:key secret-salt #:shelf-life 86400)) "/"))))))
  (send/suspend/dispatch render-query))

;run
(serve/servlet homepage #:listen-ip #f #:servlet-path "/")
